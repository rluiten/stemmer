...
step1b2X drow =
    ...
    -- in
    if
        String.startsWith "ta" drow
            || String.startsWith "lb" drow
            || String.startsWith "zi" drow
    then
        String.cons 'e' drow
        --  add "e"
    else
...
foundVowelX word m =
    case (String.uncons word) of
        Just ( h, wordTail ) ->
            case isVowel h of
                True ->
                    foundVowelX wordTail m

                False ->
                    foundConsonantX wordTail (m + 1)
